      ...
      <aside className={cn(
        ...
        <nav className="flex-1 p-3 overflow-y-auto">
          {navItems.map((item, idx) => {
            if (item.divider) return (
              <div key={idx} className="px-3 pt-4 pb-1 text-[9px] font-montserrat font-bold text-muted-foreground/50 uppercase tracking-[1.5px]">
                {item.label}
              </div>
            );
            ...
            return (
              <Link
                ...
                className={cn(
                  "flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-montserrat font-medium transition-all group mb-0.5",
        ...
        <div className="p-4 border-t border-border">
          <div className="text-[10px] text-muted-foreground font-montserrat">
            5SENSESBEAUTY LIMITED
          </div>
          <div className="text-[10px] text-muted-foreground/60 font-montserrat mt-0.5">
            Simplex-ity © 2026
          </div>
        </div>
      </aside>
